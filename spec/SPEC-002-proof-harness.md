...
| AC-4  | REQ-3    | test          | `supabase/tests/intent/wrong-helper.mutation.test.sql` — a **semantic** defect (the membership helper drops its `user_id` check) is caught by the intent layer _and confirmed green by the generated layer_, proving the two are not redundant. Reproduced in the spike. | planned  |
