...
**Every shape, and the criterion says so since 2026-09-14.** AC-4 read "an unauthorized action fails
the gate", and that was satisfiable — and satisfied — by a rule matching `export async function` and
nothing else, while `export const x = async () => {}` escaped entirely (F-78). The requirement was
never narrow; the criterion described **a test existing** rather than **a property holding**, and
`done` meant less than a reader would assume. An exported action is a public POST endpoint in every
syntax, so the criterion now names them.
...
| AC-4  | REQ-3        | test   | `scripts/check-boundaries.test.mts` — an unauthorized action fails the gate in **every shape an export is written in** (declaration, `const` arrow, function expression, default, export list), one authorizing action does not launder its neighbours, and authorizing THROUGH a local arrow helper is not a false positive | **done**         |
