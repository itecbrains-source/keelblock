...
| DEF-007 | App Router conventions ADR | Promised as ADR-010 and then displaced when ADR-010 became internationalisation — so it is currently owed and unrecorded, which is why it is here. It covers what nothing else does: project structure, Server Action conventions, where validation lives, and the typed path from schema to component. It is needed **before** the first real UI is written, not after, or the conventions get set by whatever the first screen happened to do. | `decided:app_router_conventions` |
