...
| AC-7 | REQ-7    | test          | Playbook gates wired, with a proof for **each of the five rules** this REQ names — the row previously cited three, which is F-62: no REQ without an AC and no AC verifying a non-existent REQ, both in `scripts/check-contracts.test.mts` (each mutation-proven, one against a real spec); an orphan `@defer` and a trigger-less entry in `scripts/check-deferrals.test.mts`; a dead source path in `scripts/check-research.test.mts` | **done** |
