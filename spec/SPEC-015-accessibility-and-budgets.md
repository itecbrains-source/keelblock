...
| AC-3 | REQ-3    | test   | `scripts/budget.test.mts` — the shipped tag list is asserted to be exactly WCAG 2.1 A and AA, and `best-practice` asserted absent WITH its reason still present; `docs/PRODUCT.md` B-7 now states the coverage bound rather than the word "accessible"                                            | **done** |
| AC-4 | REQ-4    | test   | `scripts/budget.test.mts` — DEF-033 must exist, name EN 301 549 and be `decided:` rather than `date:`; proven by widening the shipped tag list, which fails two assertions, so a silent edit of either side cannot pass                                                                           | **done** |
...
| AC-6 | REQ-6    | test   | `scripts/budget.mjs` and `scripts/budget.test.mts` — bytes of client JS against a bound declared in `keelblock.budget.json`; every failure message carries the LAB disclaimer, an empty build fails rather than passing, and a planted 162 KiB string took the total to 849 KiB and turned it red | **done** |
