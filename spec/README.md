...
| SPEC-002                               | Proof harness — four test layers, access matrix, mutation proofs                             | B-2, B-4      | 005           | **done** (four layers, the access matrix, and the anti-degradation rule; DEF-020 carries independent validation) |
...
| **Phase 2 — identity**                 |                                                                                              |               |               |
| SPEC-004                               | Auth — magic link, OAuth, session, route protection                                          | —             | 002, 011      | **done** (REQ-1..9; 3 criteria deferred — DEF-001/002/019)                                                       |
| SPEC-005                               | Organizations & roles — create, switch, settings, RBAC                                       | B-2           | 001, 002, 011 | **done** (REQ-1..7; the active organization is a view, never a boundary)                                         |
| SPEC-006                               | Invitations — invite, accept, decline, join, revoke, role change                             | B-2           | 001, 002, 011 | **done**                                                                                                         |
| **Phase 3 — money**                    |                                                                                              |               |               |
| SPEC-007                               | Billing — Stripe bills, the database entitles                                                | —             | 006           | **draft** (authored from memo 11; not built)                                                                     |
...
| SPEC-031                               | Erasure & portability — **account deletion** and **data export**                             | **B-2**       | 001, 005, 007 | planned                                                                                                          |
...
| SPEC-013                               | Upgrade path — the ownership boundary, applied and proven in CI                              | B-10          | 008           | **done** (the `upgrade` job scaffolds at the previous tag and runs today's suite green; B-10 claimed)            |
...
| SPEC-024                               | Handover — the trial that measures whether the repository teaches                            | **B-11**      | 002, 003      | **partial** (agent trial run and recorded; human half is DEF-024)                                                |
