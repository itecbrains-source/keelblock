-- Organization creation moves from an INSERT policy to a SECURITY DEFINER RPC.
--
...
-- spam vector (any authenticated user could create unlimited organizations), and it made the access
-- matrix cry wolf: the prober inserted an organization as an outsider, the trigger made them its
-- owner, and the resulting legitimate self-access rendered as "REACHABLE by a different
-- organization". A matrix with a false positive in it trains people to ignore the true ones.
--
-- Verified before changing: an outsider could never read anyone else's organization. The isolation
...
-- Creating an organization and becoming its owner is ONE act, so it is one transaction with one
