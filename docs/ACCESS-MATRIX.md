...
| `bypass:organization` | PARTIALLY CLOSED 2026-09-08, remainder accepted. The flagged INSERT half is gone: 20260908130000 revoked the vestigial INSERT grant, so create_organization() really is the only way a row appears. UPDATE and DELETE remain granted to authenticated on purpose -- both are policy-gated (is_org_admin / owner) and are how a tenant administers itself, not a creation path. Narrowing those to RPCs is a design change, tracked as DEF-014. |
