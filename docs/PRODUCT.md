...
| B-7  | **Accessible**: keyboard-complete, and clean at WCAG 2.1 AA on every shipped surface — automation finds most issues by volume and only a fifth to a third of the success criteria, so the keyboard half is the majority of the claim rather than a courtesy (memo 17) | An automated axe pass in CI plus a manual keyboard walkthrough per surface                                                                                                       |
| B-8  | **Fast**: a performance budget that fails the build, not a Lighthouse screenshot — a LAB budget in bytes, which predicts field performance rather than measuring it; Core Web Vitals need real users this project has not got (DEF-034)                               | Budget asserted in CI against the built app                                                                                                                                      |
