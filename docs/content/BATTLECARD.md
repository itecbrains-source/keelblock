...
## Ask where the entitlement is read, and how long a cancellation takes to bite

**What keelblock claims.** A cancellation takes effect on the next request, because the entitlement is a row read by a policy rather than a claim carried in a token.

**What the field does instead.** NOT MEASURED, and marked so rather than left to read as if it were. No competitor's billing implementation was read for this entry — unlike SPEC-001, whose argument quotes BoxyHQ's actual code. The contrast drawn is between two SHAPES: an entitlement resolved at sign-in and carried in the token or session, versus a row read by a policy per request. What is measured is keelblock's half — memo 08 measured a Supabase access token at up to an hour, which is the window the first shape has and the second does not.
...
**State.** SPEC-007 is `partial` · 8 requirements · 11 of 11 criteria met. Verify: `npm run check`.
