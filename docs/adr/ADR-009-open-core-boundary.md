...
SPEC-003 describe, in keel's exact stack: org-isolation pgTAP tagged SOC2 CC6.1, role-boundary and
...
Its own `GTM_PLAN.md` prices it at **$149–299 one-time** and plans a public/private split. keel is
...
toolkit until after they have been burned. keel's is justification — it needs a reason to be chosen
...
**The anti-degradation rule, and it is mechanised, not promised:**

> keel's full claim — B-2, isolation proven per table × command × identity, with a published access
> matrix — must hold with **zero paid components present**. A gate asserts this: the proof suite runs
> green, and the access matrix generates, from a clean checkout containing nothing but keel.

The paid layer therefore **reads keel's outputs and never patches keel's internals**. It consumes the
...
1. **The toolkit is React 18 / Node ≥20 vintage.** keel is React 19 / Next 16 / Node 26, and Cache
