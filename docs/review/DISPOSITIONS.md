...
| R-13 | implemented | `.github/workflows/check.yml`, `.github/workflows/nightly.yml`                      | Run 34190042576, 2026-09-08: the first green CI run this project has had, on a real remote. Not green first time — four runs found a privilege boundary that depended on which Postgres image the CLI pulled (F-31), a gate leaving pgTAP behind for the next one to trip over, and an unpinned code generator. DEF-003 is closed, and this register refused to let the row stay "deferred" against it. |
