...
// The review apparatus is imported WHERE IT IS USED, not at module load. A project generated by
// `create-keelblock-app` does not ship `docs/review/` — it is a review of keelblock, and every record
// in it cites a commit in keelblock's history, so it is unportable by construction (SPEC-011,
// research/13-SCAFFOLDING.md). Its readers are not shipped either, and a static import of a file
// that is not there kills this gate before it checks anything.
const reviewApparatus = () =>
  Promise.all([import('./review-register.mjs'), import('./review-records.mjs')]);
...
export function parseBars(markdown) {
  // An example bar row inside a code fence is a picture of the table, not a promise anyone made.
  return [...stripFences(markdown).matchAll(/^\|\s*(B-\d+)\s*\|\s*(.+?)\s*\|/gm)].map(
    ([, id, claim]) => ({
      id,
...
export function parseCoverage(markdown) {
  ...
  const map = {};
  for (const m of stripFences(markdown).matchAll(/^\|\s*(B-\d+)\s*\|\s*([^|]+?)\s*\|/gm)) {
    map[m[1]] = m[2]
      .split(',')
      .map((s) => s.trim())
...
/**
 * Is this a project generated by `create-keelblock-app` rather than keelblock itself?
 *
 * ONE resolver, exported, because three gates need the same answer and two answers that drift apart
 * is this repository's recurring defect. Present-shaped in every use: a generated project SAYS so.
 */
export const isGeneratedProject = () => existsSync('keelblock.provenance.json');

/**
 * Does the external-review rule apply to THIS project, and if not, why not?
 *
 * Pure, and exported, because the answer is a safety property: before SPEC-011 the condition was
 * `if (existsSync(AUDIT))`, so deleting `docs/review/` stopped the whole rule with nothing said —
 * F-41 and F-48 inside a gate whose job is to catch exactly that. The decision is now made by
 * something PRESENT: a generated project declares itself, and a project that does not declare
 * itself owes an audit.
 *
 * @param {{auditExists: boolean, generated: boolean}} state
 * @returns {{applies: boolean, reason: 'generated' | 'missing-audit' | null}}
 */
export function reviewApplicability({ auditExists, generated }) {
  if (generated) return { applies: false, reason: 'generated' };
  if (!auditExists) return { applies: false, reason: 'missing-audit' };
...
async function main() {
  ...
  //
  // Whether this rule applies is decided by something PRESENT, never by something missing. A
  // generated project SAYS it is generated, in a file this gate reads; keelblock itself does not,
  // and for keelblock a missing audit is a failure rather than a quiet skip. Before SPEC-011 the
  // condition was `if (existsSync(AUDIT))` alone — delete the directory and the whole review rule
  // stopped running with nothing said, which is F-41 and F-48 in a gate that checks for them.
  const AUDIT = 'docs/review/01-AUDIT.md';
  const REGISTER = 'docs/review/DISPOSITIONS.md';
  const verdict = reviewApplicability({
    auditExists: existsSync(AUDIT),
    generated: isGeneratedProject(),
  });
  if (verdict.reason === 'generated') {
    console.log(
      '  · review: not applicable — this project was generated by create-keelblock-app, and a ' +
        'review of keelblock is not a claim it can carry',
    );
  } else {
    ...
    const [
      { checkRegister, parseDispositions, parseFindings, summarize },
      { checkRecords, checkScoreClaims, isRecordFile, parseRecord, summarizeRecords },
    ] = await reviewApparatus();
