...
const real = readdirSync('spec').filter((f) => /^SPEC-\d+/.test(f))
  .map((f) => parseSpec(f.slice(0, 8), readFileSync(`spec/${f}`, 'utf8')));
const spec = (over = {}) => ({ id: 'SPEC-001', status: 'done', contracts: [], evidence: [], ...over });
...
describe('contracts gate', () => {
  ...
  it('MUTATION: a done criterion citing a file that does not exist fails', () => {
    const p = checkContracts([spec({ evidence: [{ ac: 'AC-1', path: 'gone.sql', status: 'done' }] })], no);
  ...
  it('a PLANNED criterion may cite a file that does not exist — it is a plan', () => {
    ...
    expect(checkContracts([spec({ status: 'partial', evidence: [{ ac: 'AC-9', path: 'later.ts', status: 'planned' }] })], no)).toEqual([]);
  });

  it('MUTATION: a glob is refused — it cannot be verified, so it is not evidence', () => {
    const p = checkContracts([spec({ evidence: [{ ac: 'AC-1', path: 'tests/*.sql', status: 'done' }] })], yes);
  ...
  it('MUTATION: index and spec file disagreeing about status fails', () => {
    ...
    const p = checkStatusAgreement([spec({ status: 'draft' })], '| SPEC-001 | Tenancy | B-2 | 001 | **done** |');
    expect(p[0]).toMatch(/index says "done", the spec file says "draft"/);
  });

  it('agreeing statuses pass', () => {
    expect(checkStatusAgreement([spec({ status: 'done' })], '| SPEC-001 | x | B-2 | 001 | **done** |')).toEqual([]);
  });
});
