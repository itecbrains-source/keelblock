...
const valid = {
  organization_member: { why: 'carries domain-invariant triggers', coveredBy: 'intent/003 (8 tests)' },
};
...
describe('policy probe plan', () => {
  ...
  it('MUTATION: a skip without `coveredBy` is refused — that is a hole, not a transfer', () => {
    ...
    expect(() => planProbes(['organization_member'], {
  ...
  it('a valid skip is recorded with its reason and its replacement coverage', () => {
    ...
    expect(skip[0]).toMatchObject({ table: 'organization_member', coveredBy: expect.stringContaining('intent/003') });
  });
  ...
  it('the real skip list names real test files', async () => {
    ...
    for (const path of referenced) expect(existsSync(path), `${path} is referenced but missing`).toBe(true);
  });
});
