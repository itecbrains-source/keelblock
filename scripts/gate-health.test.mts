...
describe('nothing is exempt by being absent from the list', () => {
  it('every script under scripts/ is either RUN by check, or carries its own proof', () => {
    ...
    // A project generated by create-keelblock-app ships the review READERS and not the records or
    // their tests (SPEC-011 REQ-4): the records cite keelblock's commits and cannot travel, and
    // `status.mjs` imports the readers synchronously so dropping them would cost the generated
    // project `npm run status`. Their proofs are keelblock's, and they stay keelblock's. Scoped by
    // something PRESENT — the project says it was generated — so this cannot quietly exempt anything
    // here.
    if (existsSync('keelblock.provenance.json')) {
      exempt.add('review-register.mjs');
      exempt.add('review-records.mjs');
    }
